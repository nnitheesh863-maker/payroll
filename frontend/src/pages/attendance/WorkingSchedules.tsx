...
export const WorkingSchedules: React.FC = () => {
  ...
  const calculateHours = (start: string, end: string, breakHrs: number) => {
    ...
    const safeBreak = Math.max(0, isNaN(breakHrs) ? 0 : breakHrs);
    const [h1, m1] = start.split(':').map(Number);
    const [h2, m2] = end.split(':').map(Number);
    const totalMins = (h2 * 60 + m2) - (h1 * 60 + m1);
    const netHours = Math.max(0, totalMins / 60 - safeBreak);
  ...
  const handleDayChange = (idx: number, field: keyof ScheduleDayPattern, val: any) => {
    ...
    const safeVal = field === 'break_duration' ? Math.max(0, Number(val) || 0) : val;
    updatedDays[idx] = { ...updatedDays[idx], [field]: safeVal };
    ...
    const updated = {
      ...selectedSchedule,
      days: updatedDays,
      work_hours_per_week: Math.round(totalWeekly * 100) / 100,
    };
  ...
  return (
    <div className="space-y-6 animate-fade-scale">
      ...
      {/* 🌟 Top Navigation Bar with Glowing Dashboard Button & Breadcrumb */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1">
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#FAF2E8] hover:bg-[#F3E5D4] text-[#8C532B] font-bold border border-[#EADBCE] transition-all hover:scale-105 shadow-xs group"
            >
              <LayoutDashboard className="h-3.5 w-3.5 text-[#8C532B] group-hover:rotate-6 transition-transform" />
              <span>Dashboard</span>
            </Link>
            <span className="text-slate-300">&rsaquo;</span>
            <span className="text-slate-800 font-bold">Working Schedules</span>
          </div>
        ...
        {/* Action Header Buttons: Attractive Dashboard, Save, and New Schedule */}
        <div className="flex items-center gap-2.5 flex-wrap">
          
          {/* 🏠 Super Attractive Dashboard Return Button */}
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 hover:from-slate-800 hover:to-slate-700 text-white text-xs font-extrabold px-3.5 py-2 rounded-xl transition-all shadow-md shadow-slate-900/20 border border-slate-700 hover:border-amber-500/50 active:scale-95 group"
          >
            <LayoutDashboard className="h-3.5 w-3.5 text-amber-400 group-hover:scale-110 group-hover:rotate-12 transition-transform" />
            <span>Dashboard</span>
          </Link>

          {/* 💾 Super Attractive Save Schedule Button */}
          <button
            onClick={handleSaveSelectedSchedule}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-[#8C532B] via-[#A06439] to-[#C28556] hover:from-[#7B3F1B] hover:to-[#A3693D] text-white text-xs font-black px-4 py-2 rounded-xl transition-all shadow-lg shadow-[#8C532B]/30 hover:shadow-[#8C532B]/50 active:scale-95 border border-white/20 cursor-pointer"
          >
          ...
          <Button
            ...
            className="bg-[#8C532B] hover:bg-[#7B3F1B] text-white shadow-md shadow-[#8C532B]/20"
          >
            New Schedule
          </Button>
        </div>
      ...
      {/* Save Success Banner */}
      {isSavedSuccess && (
        <div className="p-3.5 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-800 text-xs font-bold flex items-center justify-between shadow-xs animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0" />
            <span>Working Schedule &amp; Weekly Pattern definition saved successfully!</span>
          </div>
          <Link
            to="/dashboard"
      ...
      {viewMode === 'list' ? (
        ...
        selectedSchedule && (
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-soft space-y-6 animate-slide-right">
                        ...
                        <div className="flex items-center gap-4 flex-1 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) => handleDayChange(idx, 'break_duration', Math.max(0, Number(e.target.value) || 0))}
                              className="w-16 bg-white border border-slate-300 rounded px-2 py-1 font-mono text-xs text-slate-900 font-bold focus:border-[#8C532B] focus:outline-none"
                            />
                          </div>
                        </div>
                      ) : (
            ...
            {/* Save Working Schedule Action Bar */}
              ...
              <div className="text-xs text-slate-500">
                {isSavedSuccess ? (
                  <span className="text-emerald-600 font-bold flex items-center gap-1.5 animate-in fade-in">
                    <CheckCircle2 className="h-4 w-4" /> Working Schedule pattern saved successfully!
                  </span>
                ) : (
                  <span>Weekly modifications update derived shift hours automatically.</span>
                )}
              </div>
